...
def get_pie_chart(selected_site):      #input arguments of the callback --> values of the input in the order they are specified
    ...
    if selected_site == 'ALL':
        ...
        pie_color_dict = {'KSC LC-39A':'#636EFA', 'CCAFS LC-40':'#DC6C5A', 'VAFB SLC-4E':'#03CB97', 'CCAFS SLC-40':'#AA63FA'}
    else:
        ...
        pie_color_dict = {'Successful Launch':'#03CB97','Unsuccessful Launch':'#DC6C5A'}
    #Plot the figure
    success_pie = px.pie(
        pie_data,
        values = 'class',
        ...
        title = pie_title,
        color = pie_categories,
        color_discrete_map = pie_color_dict
        )
    return success_pie
...
def get_scatter_chart(selected_site, selected_payload_range):
    ...
    success_scatter = px.scatter(
        ...
        labels={'Payload Mass (kg)': 'Payload Mass in kg','class':'Launch outcome (0= Failed; 1= Succeeded)','Booster Version Category':'Booster Version'}
        )
    success_scatter.update_yaxes(nticks=2)
    success_scatter.update_traces(marker=dict(size=12))
